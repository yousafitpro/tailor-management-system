    ...
    <div class="bg-white rounded shadow overflow-hidden max-w-3xl">
      <form @submit.prevent="submit" autocomplete="off">
        ...
        <div class="px-8 py-4 bg-gray-100 border-t border-gray-200 flex justify-end items-center">
          <loading-button :loading="sending" class="btn-gray" type="submit">{{ $t('create_x', { x: $tc('User') }) }}</loading-button>
        </div>
      </form>
    </div>
  </div>
...
export default {
  layout: Layout,
  remember: 'form',
  components: { FileInput, TextInput, LoadingButton, CheckboxInput },
  metaInfo() {
    return { title: this.$t('create_x', { x: this.$tc('User') }) };
  },
  data() {
    return {
      sending: false,
