    ...
    <div class="w-full max-w-md no-drag">
      ...
      <div class="flex justify-center mx-auto text-white fill-white">
        <div class="h-8 italic text-white fill-white text-lg py-1 flex items-center">
          <Icons name="logo" className="w-6 h-6 ltr:mr-2 rtl:ml-2 fill-gray-300 group-hover:text-white fill-white" />
      ...
      <form class="mt-6 bg-white rounded-lg shadow-xl overflow-hidden" @submit.prevent="submit">
        <div class="p-6">
          <flash-messages />
          <p>{{ $t('Please type email to get reset instructions') }}</p>
        ...
        <div class="px-6 py-3 bg-gray-100 border-t border-gray-200 flex justify-between items-center">
          ...
          <loading-button :loading="sending" class="btn-gray" type="submit">{{ $t('Request Reset') }}</loading-button>
        </div>
      </form>
    </div>
  </div>
...
export default {
  ...
  metaInfo() {
    return { title: this.$t('Forgot Password') };
  },
  data() {
    return {
      sending: false,
